...
//----------------------------------------------------------------
//-- 本程序为oa25 Mars 跳舞的基本程序
//-- 在GPL许可下发布
//-- 本程序程序最初的来源是Zowi和 Otto
//-- 为了华人能正常使用，特标注采用中文
...
// Activate to take callibration data from internal memory  舵机初始偏移量
#define TRIM_RR 7
#define TRIM_RL 4
#define TRIM_YR  5
#define TRIM_YL -37
...
void goingUp(int tempo);        //往上走  
void drunk (int tempo);        //醉
void noGravity(int tempo);     //无重力
void kickLeft(int tempo);     //踢左边
void kickRight(int tempo);    //踢右边
void run(int steps, int T=500);
void walk(int steps, int T=1000);    //步行
void backyard(int steps, int T=3000);   //后
void backyardSlow(int steps, int T=5000);   //后慢
void turnLeft(int steps, int T=3000);    //左转
void turnRight(int steps, int T=3000);   //右转
void moonWalkLeft(int steps, int T=1000);  //月亮左
void moonWalkRight(int steps, int T=1000);  //月亮右
void crusaito(int steps, int T=1000);
void swing(int steps, int T=1000);     //摇摆
void upDown(int steps, int T=1000);    //上下
void flapping(int steps, int T=1000);   //拍动
...
void dance(){
  ...
  for(int i=0;i<4;i++) servo[i].SetPosition(90);  //设置初始位置
}
...
unsigned long final_time;     //最后时间
unsigned long interval_time;   //间隔时间
int oneTime;
int iteration;
float increment[N_SERVOS]; 
int oldPosition[]={90,90,90,90};  //初始位置 {90,90,90,90}
...
void kickLeft(int tempo){
  for(int i=0;i<4;i++) servo[i].SetPosition(90);
  delay(tempo);
  servo[0].SetPosition(50); //pie derecho     右脚
  servo[1].SetPosition(70); //pie izquiero   左脚
  delay(tempo);
  servo[0].SetPosition(80); //pie derecho   右脚
  servo[1].SetPosition(70); //pie izquiero   左脚
  delay(tempo/4);
  servo[0].SetPosition(30); //pie derecho  右脚
  servo[1].SetPosition(70); //pie izquiero  左脚
  delay(tempo/4);
  servo[0].SetPosition(80); //pie derecho  右脚
  servo[1].SetPosition(70); //pie izquiero  左脚
